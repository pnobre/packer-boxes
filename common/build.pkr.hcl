...
build {
  ...
  provisioner "powershell" {
    ...
    inline = [
      ...
      "& C:\\Windows\\Temp\\provision.ps1 -Locale \"${var.locale}\" -TimeZone \"${var.timezone}\" ${source.type == "virtualbox-iso" ? "-Hypervisor virtualbox" : "-Hypervisor vmware"} ${lower(var.theme) == "dark" ? "-UseDarkTheme" : ""}",
